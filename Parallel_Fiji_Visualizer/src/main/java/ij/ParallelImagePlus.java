...
public class ParallelImagePlus extends ImagePlus implements ImageObserver, Measurements, Cloneable {
    ...
    public ParallelImagePlus() {
		title = (this instanceof ParallelCompositeImage)?"composite":"null";
		setID();
    }
    ...
    public ParallelImagePlus(String title, Image image) {
		this.title = title;
		if (image!=null)
			setImage(image);
		setID();
    ...
    public ParallelImagePlus(String title, ImageProcessor ip) {
    ...
    public ParallelImagePlus(String pathOrURL) {
	...
	*/
	...
	public void setDisplayMode(int mode) {
		if (this instanceof ParallelCompositeImage) {
			((ParallelCompositeImage)this).setMode(mode);
			updateAndDraw();
		}
	}
	...
	public int getDisplayMode() {
		if (this instanceof ParallelCompositeImage)
			return ((ParallelCompositeImage)this).getMode();
		else
			return 0;
	}
	...
	public void setActiveChannels(String channels) {
		if (!(this instanceof ParallelCompositeImage))
			return;
		boolean[] active = ((ParallelCompositeImage)this).getActiveChannels();
	...
	/** Updates this image from the pixel data in its
		...
		The ParallelCompositeImage class overrides this method
	...
	/** Returns a reference to the current ImageProcessor. The
		ParallelCompositeImage class overrides this method to return
	...
	*/
	...
	public void show(String statusMessage) {
		...
		if ((IJ.isMacro() && ij==null) || Interpreter.isBatchMode()) {
			if (isComposite()) ((ParallelCompositeImage)this).reset();
	...
	/** Replaces this image with the specified ImagePlus. May
		not work as expected if 'imp' is a ParallelCompositeImage
	...
	public void setImage(ImagePlus imp) {
		...
		LUT[] luts = null;
		if (imp.isComposite() && (this instanceof ParallelCompositeImage)) {
			if (((ParallelCompositeImage)imp).getMode()!=((ParallelCompositeImage)this).getMode())
				((ParallelCompositeImage)this).setMode(((ParallelCompositeImage)imp).getMode());
			luts = ((ParallelCompositeImage)imp).getLuts();
		}
		LUT lut = !imp.isComposite()?imp.getProcessor().getLut():null;
		setStack(stack2, imp.getNChannels(), imp.getNSlices(), imp.getNFrames());
		ParallelCompositeImage = imp.isComposite();
		if (luts!=null) {
			((ParallelCompositeImage)this).setLuts(luts);
			((ParallelCompositeImage)this).setMode(((ParallelCompositeImage)imp).getMode());
    ...
    public void setStack(String title, ImageStack newStack) {
		...
		if (compositeChanges && (this instanceof ParallelCompositeImage)) {
			this.ParallelCompositeImage = getStackSize()!=getNSlices();
			((ParallelCompositeImage)this).completeReset();
			if (bitDepth1!=0 && bitDepth1!=getBitDepth())
				((ParallelCompositeImage)this).resetDisplayRanges();
		}
		...
		boolean invalidDimensions = (isDisplayedHyperStack()||(this instanceof ParallelCompositeImage)) && (win instanceof StackWindow) && !((StackWindow)win).validDimensions();
		if (newStackSize>1 && !(win instanceof StackWindow)) {
			...
			if (isComposite()) {
				((ParallelCompositeImage)this).reset();
				updateAndDraw();
			}
			repaintWindow();
		}
	...
	*/
	...
	public synchronized void setSlice(int n) {
		...
		if (n>=1 && n<=stack.size()) {
			...
			if (ParallelCompositeImage && getCompositeMode()==IJ.COMPOSITE && ip!=null) {
				int channel = getC();
				if (channel>0 && channel<=getNChannels())
					ip.setLut(((ParallelCompositeImage)this).getChannelLut(channel));
			}
			if (win!=null && win instanceof StackWindow)
				((StackWindow)win).updateSliceSelector();
    ...
    public FileInfo getFileInfo() {
    	...
    	switch (imageType) {
			case GRAY8: case COLOR_256:
				...
				break;
	    	case GRAY16:
	    		if (ParallelCompositeImage && fi.nImages==3) {
	...
	*/
	...
	public synchronized void flush() {
		...
		if (stack!=null && stack.viewers(-1)<=0) {
			...
			if (isComposite())
				((ParallelCompositeImage)this).setChannelsUpdated(); //flush
		}
		setStackNull();
		img = null;
		win = null;
    ...
    */
	...
	/** Returns true if this is a ParallelCompositeImage. */
	public boolean isComposite() {
		return ParallelCompositeImage && nChannels>=1 && imageType!=COLOR_RGB && (this instanceof ParallelCompositeImage);
	}

	/** Returns the display mode (IJ.COMPOSITE, IJ.COLOR
		or IJ.GRAYSCALE) if this is a ParallelCompositeImage, otherwise returns -1. */
	...
	public ParallelImagePlus flatten() {
		if (IJ.debugMode) IJ.log("flatten");
		ParallelImagePlus impCopy = this;
		if (getStackSize()>1)
			impCopy = (ParallelImagePlus) crop("whole-slice");
		ParallelImagePlus imp2 = (ParallelImagePlus) impCopy.createImagePlus();
	...
	*/
	...
	public synchronized Object clone() {
		//try {
			ParallelImagePlus copy = (ParallelImagePlus)super.clone();
			copy.win = null;
			return copy;
