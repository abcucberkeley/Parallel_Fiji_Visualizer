...
public class ParallelFolderOpener implements PlugIn {
	...
	public static ImagePlus open(String path, String rPath) {
		return open(path, null, rPath);
	}
	...
	/** Opens the images in the specified directory as a stack. Opens
		...
		argument is null. */
	public static ImagePlus open(String path, String options, String rPath) {
		ParallelFolderOpener fo = new ParallelFolderOpener();
		fo.saveImage = true;
	...
	*/
	...
	public void run(String arg, String path) {
		...
		try {
			for (int i=0; i<list.length; i++) {
				...
				else imp = opener.openImage(directory, list[i]);
			...
			*/
			...
			for (int i=this.start-1; i<list.length; i++) {
				...
				if (openAsVirtualStack) { 
					if (fileInfoStack)
						//Matt
						openAsFileInfoStack((FileInfoVirtualStack)stack, directory+File.separator+list[i]);
					else
		...
		if (stack!=null && stack.size()>0) {
			...
			else imp2 = new ImagePlus(title, stack);
			*/
	...
	private void openAsFileInfoStack(FileInfoVirtualStack stack, String path) {
		...
		if(info == null) {
			info = new FileInfo[1];
			info[0] = new FileInfo();
			info[0].fileFormat = FileInfo.UNKNOWN;
			File cFile = new File(path); 
			info[0].fileName = cFile.getName();
			info[0].directory = directory;
		}
